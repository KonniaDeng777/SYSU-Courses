...
class Execute extends Module {
  ...
  */
  ...
  io.csr_reg_write_data:=io.csr_reg_read_data
  when(funct3 === InstructionsTypeCSR.csrrw){
    io.csr_reg_write_data:=io.reg1_data
  }.elsewhen(funct3 === InstructionsTypeCSR.csrrs){
    io.csr_reg_write_data:=io.csr_reg_read_data. |(io.reg1_data)
  }.elsewhen(funct3 === InstructionsTypeCSR.csrrc){
    io.csr_reg_write_data:=io.csr_reg_read_data.&((~io.reg1_data).asUInt)
  }.elsewhen(funct3 === InstructionsTypeCSR.csrrwi){
    io.csr_reg_write_data:=Cat(0.U(27.W),uimm)
  }.elsewhen(funct3 === InstructionsTypeCSR.csrrsi){
    io.csr_reg_write_data:=io.csr_reg_read_data.|(Cat(0.U(27.W),uimm))
  }.elsewhen(funct3 === InstructionsTypeCSR.csrrci){
    io.csr_reg_write_data:=io.csr_reg_read_data.&((~Cat(0.U(27.W),uimm)).asUInt)
  }
}
