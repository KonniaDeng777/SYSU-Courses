...
app.config['MYSQL_PASSWORD'] = '20220312cz'
...
def register():
    ...
    if request.method == 'POST':
        if form.validate():
            ...
            guest_id = cur.execute('select * from guest') + 1
            ...
            cur.execute(
                'insert into guest (g_id,name,password,phone,e_mail,country) values (%s,%s,%s,%s,%s,%s)',
                [guest_id,form.name.data, password, form.phone.data, form.e_mail.data, form.country.data])
...
def book():
    ...
    if form.validate_on_submit():
        ...
        cur.execute(
            'select * from room where r_id not in (select r_id from booking where `from_`<%s and `to_`>%s)',
            [check_out, check_in])
        rooms = cur.fetchall()
...
def select_rooms():
    ...
    if len(rooms) > 0:
        ...
        cur = mysql.connection.cursor()
        for r in rooms:
            booking_id = cur.execute('select * from booking') + 1
            cur.execute(
                'insert into booking (`booking_id`,`r_id`,`g_id`,`from_`,`to_`,`meal`,`num_adult`,`num_child`,`is_paid`) values (%s,%s,%s,%s,%s,%s,%s,%s, "no")',
                [booking_id,r, session.get('g_id'), check_in, check_out, session.get('meal'), session.get('num_adult'),
...
def add_room():
    ...
    if form.validate_on_submit():  # 包含了判断request的methods
        ...
        cur = mysql.connection.cursor()
        for i in range(num):
           #values.insert(0,{'r_id',cur.execute('SELECT * from room') +1})
...
def orders():
    cur = mysql.connection.cursor()
    cur.execute('select * from booking where `g_id`="{}" and `from_`>="{}"'.format(session.get('user_id'),
                                                                                  datetime.date.today()))
...
def bills_unpaid():
    cur = mysql.connection.cursor()
    # cur.execute(
    #     'select * from booking where `g_id`="{}" and `to_`<="{}" and `is_paid`="no"'.format(session.get('user_id'),
    #                                                                                        datetime.date.today()))
    cur.execute(
    'select * from booking where `g_id`="{}" and `is_paid`="no"'.format(session.get('user_id')))
    bookings = cur.fetchall()
    # print(bookings)
    num = len(bookings)
    total = 0
...
def query_room():
    ...
    if form.validate_on_submit():
        ...
        if form.begin_date.data is not None:
            cmd2 += '`to_`>"{}" and '.format(form.begin_date.data)
        if form.to_date.data is not None:
            if cmd2 == '':
                cmd2 += '`to_`>"{}" and '.format(today)
            cmd2 += '`from_`<"{}" and '.format(form.to_date.data)
        if cmd2 == '':
            cmd2 += '`from_`<="{}" and `to_`>"{}" and '.format(today, today)
...
def edit_booking(booking_id):
    ...
    if result > 0:
        ...
        if form.validate_on_submit():
            if form.check_out.data > booking['to_']:
                flash('Only before {}'.format(booking['to_']))
                return render_template('edit_booking.html', form=form)
            if form.check_in.data >= form.check_out.data:
                flash('Illegal date input')
                return render_template('edit_booking.html', form=form)
            if form.check_in.data < booking['from_']:
                flash('Only after {}'.format(booking['from_']))
                return render_template('edit_booking.html', form=form)
            values = request.form.to_dict()
            values['from_'] = values['check_in']
            values['to_'] = values['check_out']
            del values['check_in']
            del values['check_out']
            update_value('booking', values, 'booking_id={}'.format(booking_id))
            return redirect('/booking_list')
        elif request.method == 'GET':
            form.check_in.data = booking['from_']
            form.check_out.data = booking['to_']
...
def query_booking():
    ...
    if form.validate_on_submit():
        ...
        if form.check_in.data is not None:
            where += '`from_`>="{}" and '.format(form.check_in.data)
        if form.check_out.data is not None:
            where += '`to_`<="{}" and '.format(form.check_out.data)
...
def guest_orders_list(g_id):
    cur = mysql.connection.cursor()
    cur.execute('select * from booking where `g_id`="{}" and `from_`>="{}"'.format(g_id, datetime.date.today()))
    bookings = cur.fetchall()
...
def guest_bills_list(g_id):
    cur = mysql.connection.cursor()
    cur.execute(
        'select * from booking where `g_id`="{}" and `to_`<="{}" and `is_paid`="no"'.format(g_id, datetime.date.today()))
    bookings = cur.fetchall()
    result = cur.execute('select name from guest where g_id="{}"'.format(g_id))
    if result == 0:
        cur.close()
...
def rooms_list():
    ...
    for i in range(num):
        result = cur.execute(
            'select * from booking where r_id="{}" and `from_`<="{}" and `to_`>"{}"'.format(rooms[i]['r_id'], today,
                                                                                          today))
        if result > 0:
            rooms[i]['available'] = False
        else:
