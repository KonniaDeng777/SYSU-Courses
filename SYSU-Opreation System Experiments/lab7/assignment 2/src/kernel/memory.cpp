...
void  MemoryManager::FirstFit(int ProcessSize[4], enum AddressPoolType type)
{
    if (type == AddressPoolType::KERNEL)
    {
      kernelPhysical.FirstFit(ProcessSize);
    }
    else if (type == AddressPoolType::USER)
    {
      userPhysical.FirstFit(ProcessSize);
    }
}
void MemoryManager::BestFit(int ProcessSize[4],enum AddressPoolType type)  
{
    if (type == AddressPoolType::KERNEL)
    {
      kernelPhysical.BestFit(ProcessSize);
    }
    else if (type == AddressPoolType::USER)
    {
      userPhysical.BestFit(ProcessSize);
    }
}
void MemoryManager::PrintSpace(enum AddressPoolType type)
{
    if (type == AddressPoolType::KERNEL)
    {
      kernelPhysical.PrintSpace();
    }
    else if (type == AddressPoolType::USER)
    {
      userPhysical.PrintSpace();
    }
}
