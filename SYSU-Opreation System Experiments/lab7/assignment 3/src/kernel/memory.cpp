...
void Queue::enqueue(int item[2])
{
    ...
    arr[rear][0] = item[0]; // virtual
    arr[rear][1] = item[1]; // physical
    count++;    
}
...
void MemoryManager::initQueue()
{
    FIFO_Queue.init(PAGE_NUM);
}
    ...
    {      
        ...
        {	  
	        ...
	        *pte = 0; 
	        ...
	        printf("PAGE FAULT: Swap page %d with page %d \n",PageOut, PageIn);
...
void MemoryManager::FIFO_PageReplace(const int *vaddrList, int len)
{      
    printf("\n           FIFO\n");
    for(int i = 0; i < len; i++)
    {
