...
Expr*
Ast2Asg::operator()(ast::ParenthesizedExpressionContext* ctx)
{
  auto children = ctx->children;
  auto sub = self(dynamic_cast<ast::AdditiveExpressionContext*>(children[0]));
  return sub;
}

Expr*
Ast2Asg::operator()(ast::ArrayAccessExpressionContext* ctx)
{
  auto arrayExpr = self(dynamic_cast<ast::PostfixExpressionContext*>(ctx->postfixExpression()));
  auto indexExpr = self(dynamic_cast<ast::AdditiveExpressionContext*>(ctx->additiveExpression()));
...
Expr*
Ast2Asg::operator()(ast::ArrayInitializerContext* ctx)
{
  auto ret = make<InitListExpr>();
  for (auto child : ctx->initializerList()->assignmentExpression()) {
    ret->list.push_back(self(child));
  }
  return ret;
}
...
Ast2Asg::operator()(ast::AssignmentExpressionContext* ctx)
{
  if (auto p = ctx->additiveExpression())
    return self(p);
  if (auto p = ctx->relationExpression())
    return self(p);
...
Ast2Asg::operator()(ast::MultiplicativeExpressionContext* ctx)
{
  ...
  Expr* ret = nullptr;
  if (auto parenExpr = dynamic_cast<ast::ParenthesizedExpressionContext*>(children[0])) {
    ret = self(parenExpr);
  } else {
    ret = self(dynamic_cast<ast::UnaryExpressionContext*>(children[0]));
  }
  ...
  for (unsigned i = 1; i < children.size(); ++i) {
    ...
    if (auto parenExpr = dynamic_cast<ast::ParenthesizedExpressionContext*>(children[++i])) {
      node->rht = self(parenExpr);
    } else {
      node->rht = self(dynamic_cast<ast::UnaryExpressionContext*>(children[i]));
    }
    ret = node;
...
Expr*
Ast2Asg::operator()(ast::RelationExpressionContext* ctx)
{
  auto children = ctx->children;
  Expr* ret =
...
Ast2Asg::operator()(ast::IfStatementContext* ctx)
{
  if (ctx->If())
  {
    auto ifNode = make<IfStmt>();

    ifNode->cond = self(ctx->expression());
    ifNode->then = self(ctx->statement(0));

    if (ctx->Else()) {
      ifNode->else_ = self(ctx->statement(1));
    }
    return ifNode;
  }
  ABORT();
...
Ast2Asg::operator()(ast::WhileStatementContext* ctx)
{
  if(ctx->While())
  {
    auto whileNode = make<WhileStmt>();
    whileNode->cond = self(ctx->expression());
    whileNode->body = self(ctx->statement());
    return whileNode;
  }
  ABORT();
}
...
Stmt* Ast2Asg::operator()(ast::ReturnStatementContext* ctx) 
{
    auto returnNode = make<ReturnStmt>();

    if (ctx->expression()) {
        returnNode->expr = self(ctx->expression());
    }
